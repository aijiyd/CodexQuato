...
Codex 额度监控（内部项目名 CodexQuota）是一个原生 macOS 状态栏工具。Codex 启动后，它自动显示5小时和周额度；Codex 退出后，状态栏入口自动隐藏。工具不抓网页、不保存账号密钥，只调用本机 Codex 自带的只读额度协议。
...
- 状态栏继续显示 Codex 周额度；详情面板同时显示可用的5小时额度和周额度。
- 点击状态栏入口弹出菜单式面板，显示两种额度的剩余百分比、进度条、重置倒计时和重置时间，并保留可用重置卡数量及每张卡的过期时间。
- 刷新频率使用面板内两排胶囊按钮，可选择每1秒、5秒、10秒、15秒、30秒、1分钟、2分钟、5分钟或15分钟刷新。
- 5小时额度不存在时自动隐藏对应区域；面板高度随额度和卡片数量变化，点击面板外区域自动关闭。
- 剩余 61%–100% 为绿色，20%–60% 为橙色，0%–19% 为红色。
- 使用五个短条和精确百分比展示额度；例如 42% 显示2个橙色短条。
- Codex 退出时隐藏，重新启动时自动出现。
- 详情面板使用菜单行修改刷新频率、立即刷新和退出工具，并支持 `Command + R` 与 `Command + Q`。
- 首次运行默认注册为 macOS 登录项，无 Dock 图标；如需关闭，可在 macOS 系统设置的登录项中管理。
...
3. 客户端完成初始化后调用 `account/rateLimits/read`，按时长严格识别可选的300分钟额度和必需的10080分钟周额度，并读取可用的 Codex 额度重置卡。
...
自动测试覆盖双额度解析、缺失及错误数据、刷新选项分组、颜色边界、短条数量、应用识别、JSON-RPC 初始化、读取超时、子进程退出和重新连接。冒烟测试只读取本机 Codex 额度，不发起模型任务，不消耗对话额度。
...
已完成：紧凑状态栏 UI、菜单式动态详情面板、5小时与周额度、重置卡读取、Codex 生命周期跟随、面板内刷新频率选择、失败显式展示、默认登录启动、测试、打包和文档。
...
- 周额度只接受 `windowDurationMins == 10080`，5小时额度只接受 `windowDurationMins == 300`，两者不会互相替代。
- 5小时额度是可选项；Codex 不再返回该窗口时，工具隐藏对应区域并继续显示周额度。
