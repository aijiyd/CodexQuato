...
- 点击状态栏入口弹出菜单式面板，显示周额度、剩余百分比、进度条、重置倒计时、重置时间、可用重置卡数量和每张卡的过期时间；面板高度随卡片数量变化，点击面板外区域自动关闭。
...
已完成：紧凑状态栏 UI、菜单式动态详情面板、周额度与重置卡读取、Codex 生命周期跟随、可选刷新频率、失败显式展示、默认登录启动、测试、打包和文档。
